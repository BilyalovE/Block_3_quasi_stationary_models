...
TEST(Block_3, Task_QP_1)
{
	...
	/// @param Плотность закачиваемой партии, [кг/м3]
	double density = 800;
	/// @param Вязкость закачиваемой партии, [м2/с]
...
/// Задание 3. Задача QP. Учет различия временных сеток краевых условий и метода характеристик.
...
///// Задание 2. Задача QP с «импульсной» партией.Вытеснение начальной партии другой партией при неизменном расходе перекачки, «импульсная» партия полностью не замещает основную.
TEST(Block_3, Task_QP_2)
{
	...
	double density = 990;
	/// @param Вязкость закачиваемой партии, [м2/с]
	double viscosity = 19e-6;
	/// @param Вектор параметров нефти входных партий - импульсная партия 
	vector <double> input_conditions(num_parameters);
	input_conditions = { density , viscosity };
	/// @param Вектор параметров нефти входных партий - начальная (исходная) партия
	vector <double> original_conditions(num_parameters);
	original_conditions = { input_data_task_2.initial_density, input_data_task_2.initial_viscosity };
	/// @param Время моделирования, [c]
	double T = 200000;
	/// @param Время закачивания импульсной партии, [c]
	double Tin = 5 * 3600;
	/// @param j - счетчик слоев
	int j = 0;
	double sum_dt = 0;
	...
	std::vector <double> pressure_current(input_data_task_2.n);
	do {
		Transport_equation transport_equation_task_1(input_data_task_2, synthetic_time_2, j);
		if (sum_dt <= Tin) {
			for (size_t i{ 0 }; i < num_parameters; i++) {
			transport_equation_task_1.method_characteristic(buffer.current()[i], buffer.previous()[i], input_conditions[i]);
			};
			...
			for (size_t i{ 0 }; i < num_parameters; i++) {
				transport_equation_task_1.method_characteristic(buffer.current()[i], buffer.previous()[i], original_conditions[i]);
			};
		}
		/// Передаю текущий слой
		Count_pressure_Eyler_Task_1 count_pressure_Eyler_task_1(input_data_task_2, buffer.current()[0], buffer.current()[1], pressure_current);
		pressure_current = count_pressure_Eyler_task_1.count_pressure_Eyler();
		transport_equation_task_1.output_data("Output_task_2", buffer, sum_dt, pressure_current);
		buffer.advance(1);
		sum_dt += input_data_task_2.get_dt();
		j++;
	} while (sum_dt <= T);
}